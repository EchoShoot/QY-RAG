...
import { useDataflowResultParamsContext } from './params-context';
...
export const useGetPipelineResultSearchParams = () => {
  ...
  const get = (key: PipelineResultSearchParams) =>
    ctxParams
      ? (ctxParams[key as keyof typeof ctxParams] as string) || ''
      : currentQueryParameters.get(key) || '';

  const is_read_only = (
    ctxParams
      ? ctxParams[PipelineResultSearchParams.IsReadOnly] || 'false'
      : currentQueryParameters.get(PipelineResultSearchParams.IsReadOnly)
  ) as 'true' | 'false';

  return {
    type: get(PipelineResultSearchParams.Type),
    documentId: get(PipelineResultSearchParams.DocumentId),
    knowledgeId: get(PipelineResultSearchParams.KnowledgeId),
    isReadOnly: is_read_only === 'true',
    agentId: get(PipelineResultSearchParams.AgentId),
    agentTitle: get(PipelineResultSearchParams.AgentTitle),
    documentExtension: get(PipelineResultSearchParams.DocumentExtension),
    createdBy: get(PipelineResultSearchParams.CreatedBy),
  };
};
